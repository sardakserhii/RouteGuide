...
import { DbTile } from "../db/types.js";
...
export class TilePoisService {
    ...
    async loadTilePois(
        ...
        if (
            ...
            console.log(
                `[TilePoisService] Superset cache hit for tile ${tile.id}`
            );
    ...
    async getCachedPoisForTiles(
        ...
        const freshSupersetTiles = supersetTiles.filter((t: DbTile) => {
        ...
        freshSupersetTiles.forEach((t: DbTile) => supersetTileIds.add(t.id));

        // 2. Check specific cache for remaining tiles
        const remainingTileIds = tileIds.filter(
            (id) => !supersetTileIds.has(id)
        );
        ...
        if (remainingTileIds.length > 0) {
            ...
            freshSpecificTiles = specificTiles.filter((t: DbTile) => {
                const fetchedAt = new Date(t.fetched_at);
                const diffDays =
                    (Date.now() - fetchedAt.getTime()) / (1000 * 60 * 60 * 24);
                return diffDays < options.ttlDays;
        ...
        const specificTileIds = new Set(
            freshSpecificTiles.map((t: DbTile) => t.id)
        );
        ...
        if (freshSupersetTiles.length > 0) {
            const links = await this.tilesRepo.getAllPoisForTiles(
                freshSupersetTiles.map((t: DbTile) => t.id),
                allCategoriesHash
            );
            const poiIds = links.map(
                (l: { tile_id: string; poi_id: string }) => l.poi_id
            );
            // Batch get POIs
            ...
            if (poiIds.length > 0) {
                ...
                // Filter in memory
                const filtered = pois.filter((p: Poi) =>
        ...
        for (const tile of tiles) {
            if (
                !supersetTileIds.has(tile.id) &&
                !specificTileIds.has(tile.id)
            ) {
