...
function MapView() {
    ...
    // Panel expansion states - linked so only one can be expanded at a time
    ...
    // Auto-collapse route panel and expand filter panel when route is built
    useEffect(() => {
        if (route.length > 0) {
    ...
    // Handle route panel expansion - collapse filter panel when route panel expands
    const handleRoutePanelExpandedChange = (expanded) => {
        setIsRoutePanelExpanded(expanded);
        if (expanded) {
            setIsFilterPanelExpanded(false);
        }
    };

    // Handle filter panel expansion - collapse route panel when filter panel expands
    const handleFilterPanelExpandedChange = (expanded) => {
        setIsFilterPanelExpanded(expanded);
        if (expanded) {
            setIsRoutePanelExpanded(false);
        }
    };
        ...
        <div className="relative h-screen w-screen overflow-hidden">
            ...
            <aside className="hidden md:block absolute top-0 left-0 z-[1000]">
                <RoutePanel
                    ...
                    isExpanded={isRoutePanelExpanded}
                ...
                {route.length > 0 && (
                    <PoiFilter
                        ...
                        onExpandedChange={handleFilterPanelExpandedChange}
                    />
                )}
            </aside>
            ...
            <div className="absolute inset-0 z-0">
                ...
                >
                    ...
                    {(visiblePois.length ? visiblePois : pois).map(
                        (poi, index) => {
                            return (
                                <Marker
                                    key={poi.id}
                                    position={[poi.lat, poi.lon]}
                                    icon={getNumberedCategoryIcon(
                                        poi.category,
                                        index + 1
                                    )}
                                >
                ...
                {poiMetadata?.truncated && (
                    <div className="absolute bottom-4 right-4 z-[1000] bg-yellow-100 p-3 rounded-lg shadow-lg text-yellow-800 max-w-xs">
                        Showing {pois.length} places out of {poiMetadata.total}.
                        Narrow filters to see fewer.
                    </div>
                )}
            </div>
            ...
            <aside className="hidden md:flex absolute top-0 right-0 h-full items-start justify-end z-[1000] pointer-events-none">
                {showPoiList && (
                    <PoiList
                        key={`${
                            startPoint ? startPoint.join(",") : "no-start"
                        }-${pois.length}`}
                        pois={pois}
                        startPoint={startPoint}
