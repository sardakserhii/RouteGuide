    ...
    return (
      <div className="text-center py-8">
        <p className="text-gray-500">
          {t("poiList.noPlaces", "Немає доступних місць")}
        </p>
        <p className="text-sm text-gray-400 mt-2">
          {t("poiList.adjustFilters", "Спробуйте змінити фільтри")}
        </p>
      </div>
    );
  }
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-lg font-semibold text-gray-800">
            {t("poiList.title", "Місця вздовж маршруту")}
          </h3>
        ...
        <p className="text-sm text-gray-600">
          {t("poiList.subtitle", "Натисніть, щоб додати до подорожі")}
        </p>
      </div>
        ...
        <div className="sticky top-0 bg-white border border-blue-200 rounded-lg p-3 shadow-sm z-10">
          <div className="flex items-center justify-between">
            <span className="text-sm text-gray-700">
              {t("poiList.selected", "Обрано")}:{" "}
              <strong>{selectedCount}</strong>
              {selectedCount > 23 && (
                <span className="text-xs text-amber-700 ml-1">(макс 23)</span>
              )}
            </span>
            <button
            ...
            >
              {t("poiList.exportToGoogleMaps", "Експорт")} ({selectedCount})
            </button>
          </div>
        </div>
      )}
          ...
          return (
            ...
            >
              <div className="flex gap-3">
                {/* Number Badge */}
                <div className="flex-shrink-0 w-8 h-8 flex items-center justify-center bg-blue-100 text-blue-700 text-sm font-bold rounded-full">
                  {index + 1}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    <span className="text-xs font-semibold text-blue-700 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100">
                      ...
                      {t(`categories.${poi.category}`, poi.category || "Other")}
                    </span>
                  ...
                  {/* Action Button */}
                  <button
                    onClick={() => handleTogglePoiSelection(poi.id)}
                  ...
                  >
                    {isSelected
                      ? t("poiList.added", "✓ Додано")
                      : t("poiList.addToTrip", "Додати до подорожі")}
                  </button>
                </div>
              </div>
            </div>
